...
import {
  AudioLines, AudioWaveform, Bot, Captions, Contrast, Copy, Eye, Film, FolderOpen, Gauge,
  Image, Layers3, Lightbulb, Maximize2, MessageCircle, Music2, Palette, Pause, Play,
  Redo2, RotateCcw, Scissors, Settings2, Sparkles, Split, Trash2, Type, Undo2, Upload,
  Volume2, VolumeX, X, ZoomIn, ZoomOut, FlipHorizontal2, FlipVertical2, Move3D,
  ArrowLeftRight, Snowflake, Camera, CircleDot, Brush, Shapes, MoveHorizontal,
  SlidersHorizontal, ChevronLeft, Check, Mic2, MonitorPlay
} from 'lucide-react';
...
type Tool = { id:string; label:string; icon:LucideIcon; command?:string; min?:number; max?:number; step?:number; value?:number; unit?:string };
...
const CATEGORIES:Record<Category,{label:string;en:string;icon:LucideIcon}>={
...
const sideTools={
  media:{label:'الوسائط',icon:FolderOpen}, templates:{label:'قوالب',icon:Shapes}, music:{label:'موسيقى',icon:Music2},
  text:{label:'نص',icon:Type}, filters:{label:'ملصقات',icon:CircleDot}, effects:{label:'تأثيرات',icon:Sparkles},
  transitions:{label:'انتقالات',icon:ArrowLeftRight}, adjust:{label:'ضبط',icon:SlidersHorizontal}, ai:{label:'أدوات AI',icon:Bot}
} satisfies Record<Category,{label:string;icon:LucideIcon}>;
...
export function VireonStudio({projectId}:{projectId:string}){
  ...
  const applyProperty=(v:number)=>{setPropertyValue(v);if(!property)return;const map:Record<string,string>={brightness:`اضبط السطوع إلى ${v}%`,contrast:`اضبط التباين إلى ${v}%`,saturation:`اضبط التشبع إلى ${v}%`};if(property.id in map)void command(map[property.id]);};
  ...
  return <div dir={language==='ar'?'rtl':'ltr'} className="h-screen w-screen overflow-hidden bg-[#090b10] text-white font-sans select-none">
    ...
    <header className="h-16 border-b border-white/10 bg-[#0b0d12] flex items-center justify-between px-4 gap-3">
      <div className="flex items-center gap-3 min-w-0"><div className="w-9 h-9 rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-500 grid place-items-center text-lg font-black">V</div><div><div className="font-semibold tracking-wide">Vireon</div><div className="text-[10px] text-white/40 truncate">{project?.name||L('مشروع جديد','New Project')}</div></div></div>
      <div className="flex items-center gap-2"><button onClick={undo} className="p-2 rounded-lg hover:bg-white/5"><Undo2 size={18}/></button><button onClick={redo} className="p-2 rounded-lg hover:bg-white/5"><Redo2 size={18}/></button><div className="text-white/80 text-sm px-3">{L('مشروع جديد','New project')} ✎</div></div>
      <div className="flex items-center gap-2"><button onClick={()=>setSettings(true)} className="p-2 rounded-lg hover:bg-white/5"><Settings2 size={18}/></button><button className="px-3 py-2 rounded-lg bg-white/5 text-sm">☁ {L('حفظ','Save')} <span className="text-emerald-400">✓</span></button><select value="1080p" readOnly className="bg-[#141722] border border-white/10 rounded-lg px-3 py-2 text-sm"><option>1080P</option><option>4K</option></select><button onClick={()=>void exportVideo()} className="px-4 py-2 rounded-lg bg-gradient-to-r from-violet-500 to-fuchsia-500 font-semibold shadow-lg shadow-violet-900/20">{L('تصدير','Export')}</button></div>
    </header>
    <div className="h-[calc(100vh-64px)] grid grid-cols-[88px_minmax(0,1fr)_96px] gap-2 p-2">
      <aside className="rounded-2xl bg-[#11131a] border border-white/10 overflow-y-auto flex flex-col items-center py-2 gap-1">
        {Object.entries(sideTools).map(([cat,info])=>{const C=info.icon;const key=cat as Category;return <button key={cat} onClick={()=>{setCategory(key);setPanel(key==='media'?'media':key as Panel)}} className={`w-[68px] min-h-[64px] rounded-xl flex flex-col items-center justify-center gap-1 text-[11px] ${category===key?'bg-violet-500/15 text-violet-300 border border-violet-400/30':'text-white/60 hover:bg-white/5'}`}><C size={20}/><span>{language==='ar'?info.label:CATEGORIES[key].en}</span></button>})}
      ...
      <main className="min-w-0 grid grid-rows-[minmax(260px,1fr)_minmax(240px,42%)] gap-2 overflow-hidden">
        <section className="grid grid-cols-[minmax(220px,28%)_minmax(360px,1fr)_minmax(220px,28%)] gap-2 min-h-0">
          <div className="rounded-2xl bg-[#11131a] border border-white/10 overflow-hidden min-h-0">
            <div className="h-11 border-b border-white/10 flex items-center justify-between px-3"><span className="font-semibold">{category==='media'?L('الوسائط','Media'):CATEGORIES[category].label}</span><button onClick={()=>setPanel('none')} className="p-1.5 rounded-md hover:bg-white/5"><X size={16}/></button></div>
            <div className="p-3 space-y-2 overflow-y-auto h-[calc(100%-44px)]">
              ...
              {panel==='audio' && <div className="pt-2 space-y-4"><div className="text-sm font-medium">{L('مستوى الصوت','Volume')}</div><input type="range" min="0" max="200" defaultValue="100" className="w-full accent-violet-500" onChange={e=>void command(`اضبط الصوت إلى ${e.target.value}%`)}/><button onClick={mute} className="w-full py-2 rounded-lg border border-white/10">{L('كتم الصوت','Mute')}</button></div>}
          ...
          <div className="rounded-2xl bg-[#11131a] border border-white/10 overflow-hidden min-h-0">
            <div className="h-11 border-b border-white/10 flex items-center justify-between px-3"><span className="font-semibold">{panel==='audio'?L('الصوت','Audio'):panel==='adjust'?L('الضبط','Adjust'):CATEGORIES[category].label}</span><button className="p-1.5 rounded-md hover:bg-white/5"><SlidersHorizontal size={16}/></button></div>
        ...
        <section className="rounded-2xl bg-[#11131a] border border-white/10 overflow-hidden flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-auto" onClick={e=>{const rect=e.currentTarget.getBoundingClientRect();seek((e.clientX-rect.left+e.currentTarget.scrollLeft-96)/px)}}>
            <div style={{minWidth:timelineWidth}} className="p-3">
              ...
              <div className="relative" style={{height:0}}><div className="absolute top-[-${Math.max(100,tracks.length*68+50)}px] z-30" style={{left:96+playhead*px}}><div className="w-0.5 h-[${Math.max(120,tracks.length*68+80)}px] bg-red-500"/><div className="w-3 h-3 rotate-45 bg-red-500 absolute -top-1 -left-[5px]"/></div></div>
            </div>
          </div>
          <div className="h-[116px] border-t border-white/10 bg-[#0f1117] px-3 py-2 flex flex-col gap-2"><div className="flex-1 flex items-stretch gap-2 overflow-x-auto">{activeTools.map(tool=><button key={tool.id} onClick={()=>openTool(tool)} className="min-w-[76px] px-2 rounded-xl flex flex-col items-center justify-center gap-1.5 text-[10px] text-white/70 hover:bg-white/5"><span className="w-10 h-10 rounded-xl bg-white/[0.04] border border-white/5 grid place-items-center"><tool.icon size={19}/></span><span>{tool.label}</span></button>)}</div><div className="flex items-center justify-center gap-1 text-[10px] text-white/40"><span className="w-1.5 h-1.5 rounded-full bg-violet-400"/>{L('حرّك الشريط أفقيًا لرؤية المزيد من الأدوات','Swipe horizontally for more tools')}</div></div>
        </section>
      </main>
