...
function normalizeTimeline(timeline:any){timeline.tracks=Array.isArray(timeline.tracks)?timeline.tracks:[];for(const track of timeline.tracks){track.clips=Array.isArray(track.clips)?track.clips:[];track.clips.sort((a:any,b:any)=>a.startTime-b.startTime);}timeline.duration=Math.max(0,...timeline.tracks.flatMap((t:any)=>t.clips.map((c:any)=>Number(c.endTime)||0)));timeline.currentTime=Math.max(0,Math.min(timeline.duration,Number(timeline.currentTime)||0));return timeline;}
...
app.post('/api/projects/:id/timeline',(req,res)=>{
  try{
    const id=String(req.params.id); if(!getProject(id))return res.status(404).json({error:'Project not found'});
    const timeline=normalizeTimeline(structuredClone(req.body?.timeline||timelineTemplate()));
    saveTimeline(id,timeline); res.json(payload(id));
  }catch(error){console.error('timeline save failed',error);res.status(400).json({error:'Invalid timeline'});}
});
