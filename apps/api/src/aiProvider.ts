export type EditCommand={type:'split'|'delete'|'move'|'trim_start'|'trim_end'|'set_volume'|'set_speed'|'rotate'|'flip_h'|'flip_v'|'set_brightness'|'set_contrast'|'set_saturation'|'grayscale'|'fade_in'|'fade_out'|'add_text'|'set_text'|'set_effect'|'set_transition'|'noop';time?:number;startTime?:number;value?:number;clipId?:string;text?:string;effect?:string;transition?:string;message?:string};
...
export function localCommand(text:string,timeline:any,activeClipId?:string):EditCommand{
 ...
 if(/(سرعة|speed|تسريع|تبطيء)/.test(s)){const isPercent=/%/.test(s);const v=n!==undefined?(isPercent?n/100:n):1;const bounded=Math.max(.1,Math.min(100,v));return{type:'set_speed',value:bounded,clipId:clip.id,message:`ضبطت السرعة إلى ${bounded}x.`};}
 ...
 if(/(انتقال|transition)/.test(s)&&/(fade|تلاشي)/.test(s))return{type:'set_transition',transition:'fade',clipId:clip.id,value:n??.5,message:'تمت إضافة انتقال تلاشي.'};
 if(/(انتقال|transition)/.test(s)&&/(dissolve|ذوبان|مزج)/.test(s))return{type:'set_transition',transition:'dissolve',clipId:clip.id,value:n??.5,message:'تمت إضافة انتقال مزج.'};
 if(/(انتقال|transition)/.test(s)&&/(zoom|تكبير)/.test(s))return{type:'set_transition',transition:'zoom',clipId:clip.id,value:n??.5,message:'تمت إضافة انتقال تكبير.'};
 const effectRules:[RegExp,string,string][]=[
  [/(قالب|template).*(سينمائي|cinematic)/,'cinematic','القالب السينمائي'],[/(vlog)/,'vlog','قالب Vlog'],[/(ريلز|reels)/,'reels','قالب ريلز'],
  [/(تحسين الصوت|حسّن الصوت|voice enhance|noise)/,'voice-enhance','تحسين الصوت'],[/(وهج|glow)/,'glow','الوهج'],[/(حبيبات|grain|film grain)/,'grain','حبيبات الفيلم'],
  [/(ليلي|سينمائية زرقاء|night)/,'night','المعالجة الليلية'],[/(ملصق|sticker)/,'sticker','الملصق'],[/(رموز|emoji)/,'emoji','الرموز'],[/(تراكب|overlay|طبقة)/,'overlay','التراكب'],
  [/(نص متحرك|animated text)/,'animated-text','النص المتحرك'],[/(ترجمة تلقائية|auto captions|captions)/,'auto-captions','الترجمة التلقائية'],[/(حذف الصمت|قص الصمت|silence)/,'silence-cut','قص الصمت'],
  [/(مزامنة الإيقاع|beat sync|beats)/,'beat-sync','مزامنة الإيقاع'],[/(تحسين الجودة|enhance|upscale)/,'enhance','تحسين الجودة'],[/(حركة كاميرا|camera motion)/,'camera-motion','حركة الكاميرا'],
  [/(تتبع الحركة|tracking|motion track)/,'tracking','تتبع الحركة'],[/(تسجيل|تعليق صوتي|voiceover)/,'voiceover','التعليق الصوتي'],[/(تجميد|freeze)/,'freeze','تثبيت الإطار'],[/(لقطة|snapshot)/,'snapshot','لقطة الإطار'],[/(مفتاح AI|ai)/,'ai-pass','أداة الذكاء الاصطناعي']
 ];
...
export async function parseWithOpenAI(text:string,timeline:any,fallback:EditCommand,activeClipId?:string):Promise<EditCommand>{
 const clip=clipFor(timeline,activeClipId);const local=localCommand(text,timeline,activeClipId);const base=process.env.AI_BASE_URL||'https://api.openai.com/v1';const key=process.env.AI_API_KEY||process.env.OPENAI_API_KEY;const model=process.env.AI_MODEL||process.env.OPENAI_MODEL;if(!key||!model)return local.type==='noop'?fallback:local;
