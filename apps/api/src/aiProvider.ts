...
function numberFrom(text:string):number|undefined {
  const m=text.match(/(\d+(?:[.,]\d+)?)/); return m ? Number(m[1].replace(',','.')) : undefined;
}

function clipFor(timeline:any, activeClipId?:string){
  const clips=timeline?.tracks?.find((t:any)=>t.type==='video')?.clips||[];
  return clips.find((c:any)=>c.id===activeClipId)||clips[0];
}
...
function localCommand(text:string,timeline:any,activeClipId?:string):EditCommand {
  ...
  const clip=clipFor(timeline,activeClipId);
  ...
  if(/(قسّم|قسم|split)/.test(s)) return {type:'split',time:/نصف|نصفين|half/.test(s)?clip.startTime+clip.duration/2:(n??clip.startTime+clip.duration/2),clipId:clip.id,message:'قسّمت المقطع.'};
  ...
  if(/(آخر|النهاية|end|last)/.test(s)&&/(قص|اقطع|trim)/.test(s)&&n!==undefined) return {type:'trim_end',time:Math.max(clip.trimStart+0.01,clip.trimEnd-n),clipId:clip.id,message:`قصصت آخر ${n} ثانية.`};
  ...
  if(/(صوت|volume|مستوى)/.test(s)&&n!==undefined) return {type:'set_volume',value:Math.max(0,n>2?n/100:n),clipId:clip.id,message:`ضبطت الصوت إلى ${n}%.`};
  ...
  if(/(قلب|flip)/.test(s)&&/(أفقي|افقي|horizontal)/.test(s)) return {type:'flip_h',clipId:clip.id,message:'قلبت الفيديو أفقيًا.'};
  if(/(قلب|flip)/.test(s)&&/(رأسي|راسي|vertical)/.test(s)) return {type:'flip_v',clipId:clip.id,message:'قلبت الفيديو رأسيًا.'};
  ...
  if(/(أبيض وأسود|ابيض واسود|grayscale|black and white)/.test(s)) return {type:'grayscale',clipId:clip.id,message:'تم تحويل الفيديو إلى أبيض وأسود.'};
...
export async function parseWithOpenAI(text:string,timeline:any,fallback:EditCommand,activeClipId?:string):Promise<EditCommand>{
  const clip=clipFor(timeline,activeClipId); const local=localCommand(text,timeline,activeClipId);
  const base=process.env.AI_BASE_URL||'https://api.openai.com/v1'; const key=process.env.AI_API_KEY||process.env.OPENAI_API_KEY; const model=process.env.AI_MODEL||process.env.OPENAI_MODEL;
  if(!key||!model) return local.type==='noop'?fallback:local;
  const tool={type:'function',name:'edit_timeline',description:'Edit active video clip.',parameters:{type:'object',properties:{type:{type:'string',enum:['split','delete','move','trim_start','trim_end','set_volume','set_speed','rotate','flip_h','flip_v','set_brightness','set_contrast','set_saturation','grayscale','fade_in','fade_out','noop']},time:{type:'number'},startTime:{type:'number'},value:{type:'number'},clipId:{type:'string'}},required:['type'],additionalProperties:false},strict:true};
  try{
    const r=await fetch(`${base.replace(/\/$/,'')}/responses`,{method:'POST',headers:{'content-type':'application/json','authorization':`Bearer ${key}`},body:JSON.stringify({model,input:[{role:'system',content:'You are a professional video editor command parser. Understand Arabic and English. Return one edit_timeline function call. Resolve relative requests using active clip metadata.'},{role:'user',content:`Active clip: ${JSON.stringify(clip||{})}\nRequest: ${text}`}],tools:[tool],tool_choice:{type:'function',name:'edit_timeline'}})});
    if(!r.ok)return local; const data:any=await r.json(); const call=(data.output||[]).find((x:any)=>x.type==='function_call'&&x.name==='edit_timeline'); if(!call)return local;
    const args=JSON.parse(call.arguments||'{}'); return {...args,clipId:args.clipId||clip?.id,message:args.type==='noop'?'يمكنني تنفيذ أدوات التحرير المتاحة من شريط الأدوات.':'تم تفسير الأمر وتنفيذه.'} as EditCommand;
  }catch{return local;}
}
