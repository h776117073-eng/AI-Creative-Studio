...
export function registerAIRoute(app: Express, db: Database.Database) {
  app.post('/api/projects/:id/ai-command', async (req, res) => {
    try {
      ...
      const activeClipId=String(req.body?.clipId||clips[0]?.id||'');
      const fallback:EditCommand={type:'noop',message:'يمكنني تنفيذ أدوات التحرير المتاحة من شريط الأدوات.'};
      const command=await parseWithOpenAI(String(req.body?.text||''),timeline,fallback,activeClipId);
      if(command.type==='noop') return res.json({provider:process.env.AI_API_KEY||process.env.OPENAI_API_KEY?'openai':'local',command,timeline});
      ...
      if(command.type==='split'){
        ...
        if(!(t>target.startTime&&t<target.endTime)) return res.json({provider:'local',command:{type:'noop',message:`نقطة التقسيم ${t} ثانية خارج المقطع.`},timeline});
        const ratio=(t-target.startTime)/(target.endTime-target.startTime); const second=structuredClone(target); second.id=randomUUID(); second.startTime=t; second.endTime=target.endTime; second.duration=second.endTime-second.startTime; second.trimStart=target.trimStart+(target.trimEnd-target.trimStart)*ratio; target.endTime=t; target.duration=t-target.startTime; track.clips.splice(track.clips.indexOf(target),1,target,second);
      ...
      db.prepare('UPDATE projects SET timeline_json=?,history_json=?,history_index=?,updated_at=? WHERE id=?').run(JSON.stringify(out),JSON.stringify(bounded),bounded.length-1,new Date().toISOString(),projectId);
