...
const ROOT=resolve(process.cwd(),'../..');const EXPORTS=join(ROOT,'data','exports');mkdirSync(EXPORTS,{recursive:true});
const hasAudio=(path:string)=>{try{return execFileSync('ffprobe',['-v','error','-select_streams','a:0','-show_entries','stream=codec_type','-of','default=nw=1:nk=1',path],{encoding:'utf8',timeout:30000}).trim()==='audio';}catch{return false;}};
const escText=(text:string)=>String(text).replace(/\\/g,'\\\\').replace(/:/g,'\\:').replace(/'/g,"\\'").replace(/%/g,'\\%').replace(/\n/g,'\\n');
const atempo=(speed:number)=>{let x=Math.max(.25,Math.min(4,speed));const p:string[]=[];while(x>2){p.push('atempo=2');x/=2;}while(x<.5){p.push('atempo=0.5');x/=.5;}p.push(`atempo=${x}`);return p.join(',');};
...
export function registerAIRoute(app:Express,db:Database.Database){
  app.post('/api/projects/:id/ai-command',async(req,res)=>{
    try{
      ...
      if(command.type==='add_text'){
        ...
        const start=Math.max(0,Number(command.time??out.currentTime??0));track.clips.push({id:randomUUID(),assetId:'',name:command.text||'نص',text:command.text||'نص جديد',startTime:start,endTime:start+3,trimStart:0,trimEnd:3,duration:3,fontSize:48,color:'#ffffff',opacity:1,align:'center',position:'center',keyframes:[]});
        ...
        if(command.type==='split'){const t=Number(command.time);if(!(t>target.startTime&&t<target.endTime))return res.json({provider:'local',command:{type:'noop',message:`نقطة التقسيم ${t.toFixed(2)} ثانية خارج المقطع.`},timeline});const ratio=(t-target.startTime)/(target.endTime-target.startTime);const second=structuredClone(target);second.id=randomUUID();second.startTime=t;second.endTime=target.endTime;second.duration=second.endTime-second.startTime;second.trimStart=target.trimStart+(target.trimEnd-target.trimStart)*ratio;target.endTime=t;target.duration=t-target.startTime;owner.clips.splice(owner.clips.indexOf(target),1,target,second);}
  ...
  app.post('/api/projects/:id/render-advanced',(req,res)=>{
    try{
      ...
      const timeline=JSON.parse(p.timeline_json);const vtrack=timeline.tracks.find((t:any)=>t.type==='video');const videos=(vtrack?.clips||[]).filter((c:any)=>c.duration>0);if(!videos.length)return res.status(422).json({error:'No video clips'});
      const inputs:string[]=[];let next=0;const vfLabels:string[]=[];const afLabels:string[]=[];
      for(const clip of videos){const asset:any=db.prepare('SELECT * FROM assets WHERE id=?').get(clip.assetId);if(!asset||!existsSync(asset.path))return res.status(404).json({error:`Media missing for ${clip.name}`});const audio=hasAudio(asset.path);inputs.push('-ss',String(clip.trimStart),'-t',String(Math.max(.01,clip.trimEnd-clip.trimStart)),'-i',asset.path);const input=next;next+=audio?1:2;const speed=Math.max(.25,Math.min(4,Number(clip.speed||1)));const dur=Math.max(.01,(clip.trimEnd-clip.trimStart)/speed);const vf:string[]=['setpts=PTS-STARTPTS',`setpts=PTS/${speed}`];if(clip.flipH)vf.push('hflip');if(clip.flipV)vf.push('vflip');const rot=((Number(clip.rotate||0)%360)+360)%360;if(rot===90)vf.push('transpose=1');else if(rot===180)vf.push('hflip','vflip');else if(rot===270)vf.push('transpose=2');const br=Number(clip.brightness||0),ct=Number(clip.contrast||1),sat=Number(clip.saturation||1);if(br!==0||ct!==1||sat!==1||clip.grayscale)vf.push(`eq=brightness=${br}:contrast=${ct}:saturation=${clip.grayscale?0:sat}`);if((clip.effects||[]).includes('blur'))vf.push('boxblur=4:1');if((clip.effects||[]).includes('vignette'))vf.push('vignette=PI/4');const texts=(timeline.tracks.find((t:any)=>t.type==='text')?.clips||[]).filter((t:any)=>t.startTime<clip.endTime&&t.endTime>clip.startTime&&t.text);for(const t of texts){const ls=Math.max(0,t.startTime-clip.startTime),le=Math.min(dur,t.endTime-clip.startTime);vf.push(`drawtext=text='${escText(t.text)}':fontcolor=${t.color||'white'}:fontsize=${Number(t.fontSize||48)}:x=(w-text_w)/2:y=h*0.82:enable='between(t,${ls},${le})'`);}if(Number(clip.fadeIn||0)>0)vf.push(`fade=t=in:st=0:d=${Math.min(Number(clip.fadeIn),dur)}`);if(Number(clip.fadeOut||0)>0){const fo=Math.min(Number(clip.fadeOut),dur);vf.push(`fade=t=out:st=${Math.max(0,dur-fo)}:d=${fo}`);}vfLabels.push(`[${input}:v:0]${vf.join(',')}[v${vfLabels.length}]`);const af=['aresample=48000','asetpts=PTS-STARTPTS'];if(speed!==1)af.push(atempo(speed));af.push(`volume=${Math.max(0,Math.min(4,Number(clip.volume??1)))}`);if(Number(clip.fadeIn||0)>0)af.push(`afade=t=in:st=0:d=${Math.min(Number(clip.fadeIn),dur)}`);if(Number(clip.fadeOut||0)>0){const fo=Math.min(Number(clip.fadeOut),dur);af.push(`afade=t=out:st=${Math.max(0,dur-fo)}:d=${fo}`);}if(audio)afLabels.push(`[${input}:a:0]${af.join(',')}[a${afLabels.length}]`);else{inputs.splice(inputs.length-0,0,'');}
      }
      let filter=`${vfLabels.join(';')};${afLabels.join(';')};`;
      const vcat=vfLabels.map((_,i)=>`[v${i}]`).join('');const acat=afLabels.map((_,i)=>`[a${i}]`).join('');
      filter+=`${vcat}concat=n=${videos.length}:v=1:a=0[vout];${acat||''}`;
      if(afLabels.length)filter+=`${acat}concat=n=${afLabels.length}:v=0:a=1[aout]`;
      else {const silenceIdx=next;inputs.push('-f','lavfi','-t',String(Math.max(1,timeline.duration||1)),'-i','anullsrc=channel_layout=stereo:sample_rate=48000');filter+=`[${silenceIdx}:a]anull[aout]`;}
      ...
      if(result.status!==0||!existsSync(output))return res.status(500).json({error:'Advanced FFmpeg render failed',detail:result.stderr?.slice(-4000)});res.download(output,'ai-creative-studio-advanced.mp4',()=>{try{unlinkSync(output);}catch{}});
    }catch(error){console.error('advanced render failed',error);res.status(500).json({error:'Advanced render failed'});}
  });
}
