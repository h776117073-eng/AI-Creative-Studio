...
def run(cmd:list[str], check=True)->str:
    ...
    if check and p.returncode: raise RuntimeError(p.stderr[-4000:])
    return p.stdout
...
def inputs(path:Path,out:Path)->list[Path]:
    if path.suffix.lower()=='.xapk' or (zipfile.is_zipfile(path) and path.suffix.lower()!='.apk'): return xapk_members(path,out)
    return [path]
...
def shaders(roots):
    ...
    for root in roots:
        for p in root.rglob('*'):
            if p.is_file() and (p.suffix.lower() in SHADERS or 'shader' in p.name.lower()): rows.append({'path':rel(p,root),'size':p.stat().st_size,'sha256':digest(p)})
    return rows
...
def source_scan(roots):
    ...
    for root in roots:
        for p in root.rglob('*.java'):
            t=p.read_text(errors='ignore')
            hits={term:len(re.findall(rf'\\b{re.escape(term)}\\b',t.lower())) for term in TERMS}
    ...
    for root in roots:
        for p in root.rglob('*'):
            ...
            if p.suffix.lower() in {'.json','.json5'}:
                ...
                t=p.read_text(errors='ignore'); row['tags']=sorted(set(re.findall(r'<([A-Za-z_][A-Za-z0-9_.:-]*)\\b',t)))[:100]; row['attributes']=sorted(set(re.findall(r'\\b(?:android:)?([A-Za-z_][A-Za-z0-9_]*)="',t)))[:100]
            rows.append(row)
